...
## Where barter survives: services and pooled information

Money won so completely for *goods* that it is easy to miss where barter quietly persists — and it persists in two revealing places. The first is **services**. Consider a babysitting cooperative: a group of families who watch each other's children, paying not in cash but in **scrip**, tokens each worth a half-hour of sitting. Sit for a neighbour and you earn scrip; go out for the evening and you spend it. No money changes hands, yet the arrangement borrows money's cleverest trick — the scrip is a *medium of exchange* (it ends the hunt for a family that both wants a sitter and has a free evening) and a *unit of account* (everything is priced in half-hours). It works because a service like childcare has none of the properties money was invented to fix: it is not storable, an hour is already a common yardstick, and neighbours trade over and over, so the search cost vanishes — and paying a friend cash to mind your children would feel wrong in a way that returning the favour does not. The same logic runs the modern **time bank**, where an hour of any service trades for an hour of any other, often right alongside ordinary money.

The second, more surprising survival is in **information**, and its clearest form today is data. On Crunchbase, thousands of venture-capital firms hand over monthly updates on their investments and, in return, receive the accurate, complete funding picture the platform assembles from all of them — a swap of information for information, with no cash in the loop. Credit bureaus run the same bargain at the scale of an entire banking system: under the industry's *principles of reciprocity*, a lender may see only as much of everyone else's repayment history as it contributes of its own — refuse to share, and you are locked out. Even governments do it, exchanging weather observations freely across borders, because no nation can forecast its own skies without the readings from upwind of it.

What unites the babysitter and the venture capitalist is a single point money cannot reach: the thing each party actually wants — the covered evening, the complete dataset — **cannot be bought at any price, because it exists only if everyone contributes.** Here money is not merely clumsy; it is the wrong tool, since nothing is for sale. Barter did not die out. It retreated to exactly the exchanges — reciprocal services, and pooled information — where money has nothing to sell.
