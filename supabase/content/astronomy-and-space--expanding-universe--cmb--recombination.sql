...
Two further consistency checks make the value hard to escape. The **sound horizon appears again** in galaxy surveys, as the baryon acoustic oscillation scale imprinted in the clustering of galaxies at low redshift — the same ruler, measured 13 billion years later and at completely different wavelengths, giving the same length. And the **temperature scaling** $T \propto (1+z)$ has been checked directly: molecular absorption lines and the Sunyaev–Zel'dovich effect measured against clusters at $z \approx 1$–3 confirm the CMB was hotter in the past by the predicted factor.
...
- $z_* = 1089.92 \pm 0.25$ is **measured, not assumed**, from the acoustic peak positions, which fix $r_s = 147$ Mpc against the distance to last scattering. But $\tau = 0.054 \pm 0.007$ is poorly determined, because $e^{-2\tau}$ is **degenerate with the primordial amplitude $A_s$** — a dataset's precision does not distribute evenly across what is inferred from it.
